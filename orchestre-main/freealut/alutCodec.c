...
mulaw2linear(uint8_t mulawbyte) {
    ...
    sample = exp_lut[exponent] + (short) (mantissa << (exponent + 3));
    if (sign != 0) {
        sample = -sample;
    }
    return sample;
...
_alutCodecIMA4(ALvoid *data, size_t length, ALint numChannels,
    ...
    blocks = (size_t)(length / (size_t) blockAlign);
    buf = (int16_t *) _alutMalloc((size_t)(((size_t) blockAlign - (size_t) numChannels) * blocks * 4));
    ...
    for (i = 0; i < blocks; i++) {
        ...
        for (chn = 0; chn < (size_t) numChannels; chn++) {
            predictor[chn] = *d++;
            predictor[chn] |= *d++ << 8;
        ...
        for (j = (size_t)(numChannels * 4); j < (size_t) blockAlign;) {
            for (chn = 0; chn < (size_t) numChannels; chn++) {
                int16_t *ptr_ch;
                size_t q;
            ...
            j += (size_t)(numChannels * 4);
            ptr += numChannels * 8;
        }
    }
    free(data);
    return _alutBufferDataConstruct(buf, (size_t)(((size_t) blockAlign - (size_t) numChannels) * blocks * 4),
