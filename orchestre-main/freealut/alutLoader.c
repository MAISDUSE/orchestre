...
loadAUFile (InputStream *stream)
{
  ...
  if (!_alutInputStreamSkip (stream, (size_t) (dataOffset - AU_HEADER_SIZE)))
    {
      return AL_FALSE;
    }
                                             ...
                                             ALsizei length)
{
  ...
  stream = _alutInputStreamConstructFromMemory (data, (size_t) length);
  return _alutCreateBufferFromInputStream (stream);
}
